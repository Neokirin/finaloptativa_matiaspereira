...
class BlocksGroup(pygame.sprite.OrderedUpdates):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.speed = 1000  # Initial speed
        self.score_to_speedup = 30  # Increase speed every 30 points
    ...
    def update(self):
        if self.score % self.score_to_speedup == 0:
            self.speed -= 30
...
def main():
    ...
    level_msg_text = font.render(
        "Nivel:", True, (255, 255, 255), bgcolor)  # Texto para el nivel
    ...
    while run:
        ...
        # Calcular el nivel y mostrarlo
        level = blocks.score // 50 + 1
        level_text = font.render(
            str(level), True, (255, 255, 255), bgcolor)
        draw_centered_surface(screen, level_msg_text, 300)
        draw_centered_surface(screen, level_text, 330)

        if game_over:
            draw_centered_surface(screen, game_over_text, 360)
            add_score(player_name, blocks.score)
